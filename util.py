...
async def split_large_files(input_file):
    ...
    if check_is_streamable(input_file):
        ...
        LOGGER.info(f"Total File Size: {total_file_size}")

        # Still calculate duration logic, but skip splitting
        minimum_duration = (total_duration / total_file_size) * (Config.TG_SPLIT_SIZE)
        minimum_duration = int(minimum_duration)
        LOGGER.info(f"Minimum Duration (per chunk): {minimum_duration}")

        # Prepare output file name (single full copy)
        base_name = os.path.basename(input_file)
        input_extension = base_name.split(".")[-1]
        full_file_name = f"{base_name}_FULL.{input_extension}"
        output_file = os.path.join(new_working_directory, full_file_name)

        # Copy full file (no trimming)
        os.rename(input_file, output_file)
        LOGGER.info(f"Moved full file to: {output_file}")

    else:
        # For non-streamable files, also just move without splitting
        output_file = os.path.join(
            new_working_directory,
            os.path.basename(input_file),
        )
        os.rename(input_file, output_file)
        LOGGER.info(f"Moved non-streamable file to: {output_file}")
